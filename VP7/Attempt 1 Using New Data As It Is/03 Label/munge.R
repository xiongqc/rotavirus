...
seq <- read.dna("RVA_VP7_excLVE_Jan18_quickalign3_trim.fas",format="fasta",as.matrix=FALSE) #as.matrix = FALSE: always returns the sequences in a list.
base <- paste(rota2$accession,rota2$host2, rota2$collection_date2, "Rotavirus", rota2$organism2, "VP7", sep="_")
...
for (i in 1:6000)
  tryCatch({
{
names(seq)[i] <- base[grep(names(seq)[i],base)]
print(i)
}
  }, error=function(e){
...
write.dna(seq, "RVA_VP7_excLVE_Jan18_quickalign3_trim_label.fas", format = "fasta")