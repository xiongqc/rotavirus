...
getBEASTxml<-function(seqs,outfile){
  ...
  if(!is.na(taxa.noyear[1])){
    for(i in taxa.noyear){
      addChildren(operators,newXMLNode("randomWalkOperator",newXMLNode("parameter",attrs=c(idref=paste0("age(",nam[i],")"))),attrs=c(windowSize="1.0",weight="1")))
    }
  }
...
files<-dir(,"*.fas") #get all files in folder of .fa type
for(i in files){
  seqs<-read.dna(i,format="fasta") #read in sequences
  outfile<-gsub(".fas","",i) #names output files after original filenames, removing the .fa
